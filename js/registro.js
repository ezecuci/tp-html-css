...
function validacionDeIngreso(apellido, nombre, email, password, password2, tipo){
    ...
    if(!validador.esNombrePropioValido(apellido)){
        modal.mostrarMensaje('El apellido ingresado no es valido', () => {
        inputApellido.focus();
        });
        return false;
    }
    ...
    if(tipo === 'personal' && !validador.esNombrePropioValido(nombre)){
        modal.mostrarMensaje('El nombre ingresado no es valido', () => {
        inputNombre.focus();
        });
        return false;
    }
    ...
    if(!validador.esUnEmailValido(email)){
        modal.mostrarMensaje('el email ingresado no es valido\nUtilice el siguiente formato: usuario@ejemplo.com', () => {
        inputEmail.focus();
        });
        return false;
    }
    ...
    if(!validador.esPasswordValida(password)){
        modal.mostrarMensaje('La contraseña ingresada no es valida', () => {
        inputPassword.focus();
        });
        return false;
    }
    ...
    if(!validador.sonIguales(password, password2)){
        modal.mostrarMensaje('las contraseñas ingresadas no coinciden', () => {
        inputPassword.focus();
        });
        return false;
    }
